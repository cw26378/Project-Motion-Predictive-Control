...
using CppAD::AD;
const double MAX = 1.0e19; // std::numeric_limits<double>::max()
...
size_t delta_start = epsi_start + N; // ********* N-1 or N? *********//
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++){
      ...
      //for (int i = 0; i < coeffs.size(); i++) { f0 += coeffs[i] * CppAD::pow(x0, i);}

      AD<double> psides0 = CppAD::atan(coeffs[1] + 2 * x0 * coeffs[2] + 3 * CppAD::pow(x0, 2)* coeffs[2]);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -MAX;
    vars_upperbound[i] = MAX;
  }
